...
class BaseRepository[T](ABC):
    ...
    @abstractmethod
    def _to_domain(self, row: Record | None) -> T | None:
    ...
    def _validate_id(self, id: Any) -> UUID:
        if id is None or not isinstance(id, UUID):
            raise ValueError(f"cannot get a record without a valid id. {id}")
        return id

    @abstractmethod
    async def add(self, cmd: BaseModel, connection: Connection | None = None) -> T:
    ...
    async def update(
        ...
        for idx, col in enumerate(data_dict.keys(), start=2):
            ...
            value = data_dict[col]
            # value = (
            #     data_dict[col].get_secret_value()
            #     if isinstance(data_dict[col], SecretStr)
            #     else data_dict[col]
            # )
            values.append(value)
