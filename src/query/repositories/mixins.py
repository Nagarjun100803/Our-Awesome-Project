...
class PaginatorMixin:
    async def paginate_query(
        ...
        self.db: DBManager  # Add a type hint to get IDE support.
        ...
        result, count_of_items = await asyncio.gather(
            self.db.execute(executable, fetch_returns="all", connection=connection),
            self.db.execute(
                count_executable, fetch_returns="one", connection=connection
            ),
        )
